// FourierFit.java
//
// approximate f(x) ~ a0 + a1 cos(x) + b1 sin(x) + a2 cos(2x) + b2 sin(2x) + ...
...
class FourierFit extends Frame
{
  FourierFit()
  {
    setTitle("FourierFit(x) 0 to 2pi of step function");
    setSize(450,500);
  ...
  public void paint(Graphics g)
  {
    ...
    // compute Fourier series coefficients a's and b's
    for(int i=0; i<nt; i++)
    {
      a[i]=0.0;
      b[i]=0.0;
      ...
      a[i]=(2.0/tpi)*(xmax-xmin)*a[i]/nf; // scale numerical integration
      b[i]=(2.0/tpi)*(xmax-xmin)*b[i]/nf;
      if(i==0)a[0]=a[0]/2.0; // constant term special, no b[0]
    ...
    for(int k=0; k<4; k++)
    {
      ...
      {
        x1new=xstart+(double)i*dx;
        y1new=a[0]+a[1]*Math.cos(x1new)+b[1]*Math.sin(x1new);
        for(int j=3; j<t; j=j+2)y1new=y1new+a[j]*Math.cos((double)j*x1new)+
                                            b[j]*Math.sin((double)j*x1new);
        ax[i]=y1new;
    ...
    g.setFont(cur18);
    g.drawString("FourierFit(x) 0 to 2pi of step function", 10, 50);
  }
  
  public static void main(String args[])
  {
    System.out.println("FourierFit.java running");
    new FourierFit();
  }
}
